...
const DashboardDoctor = () => {
  ...
  const fetchConsultations = useCallback(async () => {
    ...
    try {
      ...
      if (doctor) {
        const response = await api.get(`/consultations/doctor/${doctor.id}`);
        ...
        setStats({
          ...
          completed: data.filter((c) => c.status_konsultasi === "completed")
            .length,
        });
      } else {
        setConsultations([]);
        ...
        <div className="container mx-auto px-4">
          <div className="bg-rose-50 border border-rose-200 rounded-2xl p-8 text-center">
            ...
            <button
              ...
              className="mt-4 bg-rose-600 text-white px-4 py-2 rounded-xl hover:bg-rose-700 transition"
            >
              Muat Ulang Halaman
            </button>
          </div>
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1">
            Selamat datang, dr. {user?.nama?.split(" ")[0]}
          </p>
        </div>

        {/* Stat Cards - Dalam satu baris horizontal */}
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {consultations.map((consultation) => (
                  <div
                    key={consultation.id}
                    className="border border-emerald-100 rounded-xl p-4 hover:shadow-md transition hover:border-emerald-200"
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-3">
                        <img
                          src={
                            consultation.profiles?.foto_url ||
                            `https://ui-avatars.com/api/?background=0891b2&color=fff&name=${consultation.profiles?.nama?.charAt(0) || "P"}`
                          }
                          className="w-10 h-10 rounded-full object-cover"
                          alt="Patient"
                          onError={(e) => {
                            e.target.src = `https://ui-avatars.com/api/?background=0891b2&color=fff&name=${consultation.profiles?.nama?.charAt(0) || "P"}`;
                          }}
                        />
                        <div>
                      ...
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-medium ${getStatusBadge(consultation.status_konsultasi)}`}
                      >
